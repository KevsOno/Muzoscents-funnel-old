...
(function applyPwaMobileMode() {
    ...
    // Run once after markup is available and once after DOMContentLoaded
    // for elements rendered by the storefront.
...
function productCardHtml(p, featured = false) {
    ...
    const imageHtml = p.image_url
        ? `<img src="${p.image_url}" alt="${escapeHtml(p.description)}" loading="lazy">`
...
function renderHeroProducts() {
    ...
    slots.forEach((slot, i) => {
        const p = products[i];
        slot.innerHTML = p
            ? `<img src="${p.image_url}" alt="${escapeHtml(p.description)}">`
...
function renderCategoryCards() {
    ...
    container.innerHTML = cats.map((category, index) => {
        ...
        const visual = product
            ? `<img src="${product.image_url}" alt="${escapeHtml(category)}">`
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    handleDeepLinks();
    showView('shop');
    // Hero is intentionally static in the new storefront design.
