...
function renderPagination() {
    ...
    for (let i = startPage; i <= endPage; i++) {
        // 🔸 Pagination active page – ORANGE (original)
...
function renderProducts() {
    ...
    items.forEach(p => {
        ...
        const imageHtml = safeImageUrl ?
            ...
            `<i class="fa-solid fa-box-open text-4xl text-green-700"></i>`;

        card.innerHTML = `
              <div class="h-28 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl flex items-center justify-center mb-3">${imageHtml}</div>
              <h3 class="font-bold text-gray-800 truncate text-sm md:text-base">${escapeHtml(p.description)}</h3>
              <div class="flex justify-between items-center mt-2">
                <span class="text-green-600 font-bold text-lg md:text-xl">₦${p.price_naira.toLocaleString()}</span>
              ...
              <div class="flex items-center gap-1 md:gap-2 mt-2 flex-wrap">
                ...
                <button class="qty-btn green" data-id="${p.id}" data-delta="1">+</button>
              ...
              <div class="flex gap-2 mt-3">
                ...
                <button class="add-to-cart-btn flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded-xl text-sm font-semibold" data-id="${p.id}"><i class="fa-solid fa-cart-plus"></i> Add</button>
              </div>
            `;
...
// ============================================================
//  TODAY'S PICKS – DYNAMIC CAROUSEL (2 at a time)
// ============================================================
...
function renderFeaturedProducts(products) {
    ...
    slides.forEach((chunk, idx) => {
        ...
        chunk.forEach(p => {
            ...
            const card = document.createElement('div');
            card.className = "product-card bg-white rounded-2xl p-4 shadow-sm hover:shadow-lg transition flex flex-col";
            card.innerHTML = `
                <div class="h-28 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl flex items-center justify-center mb-3">${imageHtml}</div>
                ...
                <div class="flex items-center gap-1 md:gap-2 mt-2 flex-wrap">
                    ...
                    <button class="qty-btn green" data-id="${p.inventory_id}" data-delta="1">+</button>
                    <span class="text-xs text-gray-400 ml-0 md:ml-1">× ₦${p.price_naira.toLocaleString()}</span>
                </div>
                <div class="flex gap-2 mt-3">
                    ...
                    <button class="add-to-cart-btn flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded-xl text-sm font-semibold" data-id="${p.inventory_id}">
...
function goToFeaturedSlide(index) {
    ...
    const dots = container.querySelectorAll('.featured-dot');
...
function showProductDetail(p) {
    ...
    content.innerHTML = `
        <div class="flex flex-col gap-4">
            ${safeImageUrl ? `<img src="${safeImageUrl}" class="w-full h-48 object-contain rounded-xl">` : `<div class="w-full h-48 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl flex items-center justify-center"><i class="fa-solid fa-cube text-5xl text-green-700"></i></div>`}
            <h2 class="text-2xl font-bold">${escapeHtml(p.description)}</h2>
            <div class="flex justify-between">
                <span class="text-3xl font-bold text-green-600">₦${p.price_naira.toLocaleString()}</span>
                <span class="bg-gray-100 px-3 py-1 rounded-full">Stock: ${p.current_qty||0}</span>
            </div>
            <div class="border-t pt-3 grid grid-cols-2 gap-3 text-sm">
                <div><span class="text-gray-500">Weight:</span> ${p.weight_kg ? p.weight_kg+' kg' : 'Not specified'}</div>
                <div><span class="text-gray-500">Dimensions:</span> ${dim}</div>
                <div><span class="text-gray-500">Handling:</span> ${p.handling_category || 'Standard'}</div>
                <div><span class="text-gray-500">Size factor:</span> ${p.size_factor || 'Standard'}</div>
                <div class="col-span-2 flex gap-2">${p.fragile ? '<span class="bg-red-100 text-red-700 px-2 py-1 rounded-full text-xs">Fragile</span>' : ''}${p.hazardous ? '<span class="bg-amber-100 text-amber-700 px-2 py-1 rounded-full text-xs">Hazardous</span>' : ''}</div>
                ${p.product_group ? `<div><span class="text-gray-500">Group:</span> ${escapeHtml(p.product_group)}</div>` : ''}
                ${p.item_code ? `<div><span class="text-gray-500">Item code:</span> ${escapeHtml(p.item_code)}</div>` : ''}
            </div>
            <!-- 🔸 Modal delivery note – ORANGE (original) -->
            <div class="bg-orange-50 p-3 rounded-xl text-sm">
                <i class="fa-regular fa-truck-fast text-orange-500 mr-2"></i> Delivery fees calculated based on weight, dimensions, and distance.
            </div>
        </div>
    `;
        ...
        return `<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50 p-3 rounded-xl gap-2">
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 mt-1">
                  ...
                  <button class="cart-qty-btn qty-btn green" data-index="${idx}" data-delta="1">+</button>
                  <span class="text-xs text-gray-400 ml-1">× ₦${item.price.toLocaleString()}</span>
                </div>
              </div>
              <div class="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                <span class="font-mono text-green-600 font-semibold">₦${sub.toLocaleString()}</span>
...
function showConfirmation(orderData) {
    ...
    container.innerHTML = `
                <div class="bg-gray-50 rounded-xl p-4 space-y-2">
                    <div class="flex justify-between text-sm flex-wrap"><span class="text-gray-600">Receipt Number</span><span class="font-mono font-bold text-green-600">${escapeHtml(orderData.receiptNumber || 'N/A')}</span></div>
                ...
                <div class="border-t border-gray-200 pt-3 space-y-1">
                    ...
                    <div class="flex justify-between font-bold text-lg border-t border-gray-200 pt-2 mt-1"><span>Total</span><span class="text-green-600">₦${(orderData.totalAmount || 0).toLocaleString()}</span></div>
                </div>
            `;
...
function addChatMsg(sender, text, isTypingIndicator = false) {
    ...
    div.className =
        `p-2 rounded-xl max-w-[85%] text-sm mb-1 ${sender==='user'?'bg-green-600 text-white ml-auto rounded-br-none':'bg-gray-100 text-gray-800 mr-auto rounded-bl-none'}`;
...
function setDeliveryOpt(opt) {
    ...
    if (opt === 'pickup') {
        // 🔸 Delivery toggle active – ORANGE (original)
    ...
    document.getElementById('track-submit')?.addEventListener('click', async () => {
        ...
        try {
            ...
            const statusMap = { pending: "⏳ Pending", completed: "📦 Ready", shipped: "🚚 In transit",
                delivered: "✅ Delivered" };
            resultDiv.innerHTML =
                `<div class="bg-gray-100 p-3 rounded-lg space-y-1 text-sm"><div><strong>Receipt:</strong> ${data.receipt_number}</div><div><strong>Total:</strong> ₦${data.total_amount?.toLocaleString()}</div><div><strong>Status:</strong> <span class="text-green-600 font-semibold">${statusMap[data.shipping_status] || data.shipping_status}</span></div><div><strong>Carrier:</strong> ${data.shipping_carrier || '—'}</div><div><strong>Tracking #:</strong> ${data.tracking_number || 'N/A'}</div></div>`;
