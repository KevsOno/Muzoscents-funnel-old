...
// ─── AUTO‑ESTIMATE HELPER (called after address change) ───
...
// ============================================================
//  CHECKOUT – WITH CLEAR AUTH MESSAGE
// ============================================================
async function checkout() {
    ...
    // 🔐 Check authentication with clear message
    if (!token) {
    ...
    try {
        ...
        // Only paystack is available now (cash removed)
        const items = cart.map(i => ({ inventory_id: i.id, quantity: i.quantity }));
        const subtotal = cart.reduce((s, i) => s + i.price * i.quantity, 0);
        let total = subtotal;
